...
optimizer2 = torch.optim.Adam(model_BN.parameters(),lr=lr)
criterion = nn.CrossEntropyLoss()


train_loader = get_cifar_loader(train=True)
val_loader = get_cifar_loader(train=False)
losses, val_acc = train(model, optimizer1, criterion, train_loader, val_loader, epochs_n=20)
losses_BN, val_acc_BN = train(model_BN, optimizer2, criterion, train_loader, val_loader, epochs_n=20)

plt.plot(range(len(losses)), losses,'o-', label='no BN')
plt.plot(range(len(losses_BN)), losses_BN, 'o-', label='with BN')
plt.legend()
plt.xlabel('step')
plt.ylabel('loss')
